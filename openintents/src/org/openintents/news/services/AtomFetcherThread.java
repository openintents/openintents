 ...
 */
public class AtomFetcherThread extends Thread{
	...
	public void parse(Document doc){
		...
		for (int i=0;i<nlen ;i++ )
		{
			...
			selection.append(News.AtomFeedContents.FEED_ID);
			selection.append("=");
			selection.append(_id);

			cv.put(News.AtomFeedContents.FEED_ID,_id);
			for (int n=0;n<childLen ;n++ )
			{
				...
				*/
				...
				//	System.out.println(nodeValue+"\n");
					...
					{
						cv.put(News.AtomFeedContents.ENTRY_ID,nodeValue);
						selection.append(" AND ");
						selection.append(News.AtomFeedContents.ENTRY_ID);
						selection.append(" like '");
						selection.append(nodeValue);
						selection.append("'");
					}else if (nodeName.equalsIgnoreCase("title"))
					{
						cv.put(News.AtomFeedContents.ENTRY_TITLE,nodeValue);
					}else if (nodeName.equalsIgnoreCase("summary"))
					{
						cv.put(News.AtomFeedContents.ENTRY_SUMMARY,nodeValue);
					}
				}
