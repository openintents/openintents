...
public class MediaPlaylistsProvider extends ContentProvider {
	...
	private static class PlaylistDBHelper extends SQLiteOpenHelper{
	...
	public Uri insert(Uri uri, ContentValues values) {
		...
		switch (match){
			case PLISTS:
				rowID=mDB.insert(TABLE_LISTS, "", values);			
				if (rowID > 0) {
			...
			case ITEMS:
					...
					rowID=mDB.insert(TABLE_ITEMS, "", values);			
					if (rowID > 0) {
	...
	public int update(Uri uri, ContentValues values, String selection, String[] selectionArgs) {
		...
		switch (match){
			case PLISTS:
				result= mDB.update(TABLE_LISTS, values, selection,selectionArgs);
				//getContext().getContentResolver().notifyChange(nUri, null);
			...
			case PLISTS_ID:
				...
				break;
			case ITEMS:
				result= mDB.update(TABLE_ITEMS, values, selection,selectionArgs);
				//getContext().getContentResolver().notifyChange(nUri, null);
				getContext().getContentResolver().notifyChange(uri, null);
				break;
			case ITEMS_ID:
