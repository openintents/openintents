...
public class GTalkReceiver extends IntentReceiver {
	...
	/** Array of items for editing.
	 * This defines the projection for the table ContainsFull. */
	private static final String[] mProjectionContainsFull =
		new String[]{
			ContainsFull._ID,
    ...
    public void onReceiveIntent(Context context, Intent intent) {
    	...
    	if (data.equals(Shopping.Lists.CONTENT_URI)) {
    		if (action.equals(OpenIntents.SHARE_UPDATE_ACTION)) {
    			// Update on a shopping list
    			updateList(bundle);
    			return;
    		}
    	} else if (data.equals(Shopping.Items.CONTENT_URI)) {
    		if (action.equals(OpenIntents.SHARE_INSERT_ACTION)) {
    			// Insert a new item
    			insertItem(bundle);
    			return;
    		} else if (action.equals(OpenIntents.SHARE_UPDATE_ACTION)) {
    			// Update an item
    			updateItem(bundle);
    			return;
    		}
     ...
     * @param bundle
     */
    void updateList(Bundle bundle) {
    	// Update information about list:
		if (bundle != null) {
			String shareListName = bundle.getString(Shopping.Lists.SHARE_NAME);
			String shareContacts = bundle.getString(Shopping.Lists.SHARE_CONTACTS);
			
			if (shareListName == null) {
				Log.e(TAG, "Bundle received is incomplete: shareListName is null.");
				return;
			}
			if (shareContacts == null) {
				Log.e(TAG, "Bundle received is incomplete: shareContacts is null.");
				return;
			}
	        ...
	        if (c == null || c.count() < 1) {
	    		...
	    		// The sender is responsible that the contacts list
	    		// we receive
	    		//  (1) contains the sender's email,
	    		//  (2) does not contain our email.
	    		...
	    		try {
	                ...
	                Toast.makeText(mContext, "Received new shopping list: " + shareListName, Toast.LENGTH_LONG).show();
	    		} catch (Exception e) {
	    			Log.i(TAG, "insert list failed", e);
	    		}
	        } else {
	        	// List exists, let us just update email contacts
	        	c.first();
    ...
    void insertItem(Bundle bundle) {
		...
		if (bundle != null) {
	        ...
	        if (c == null || c.count() < 1) {
	        	...
	        	// TODO: Ask user what to do about the item:
	        	// Either demand new list + synchronization, or drop item
	        	// OR: Automatically create the list with minimum entries
	        } else {
	        	// List exists, let us insert item
	        	c.first();
     ...
     * 
     * If the item does not exist yet, it is created.
     * Update could include to strike an item through.
     * 
     * @param bundle
     */
    void updateItem(Bundle bundle) {
		...
		if (bundle != null) {
			...
			if (itemNameOld == null || 
					...
					itemStatus == null || 
					itemSender == null) {
				Log.e(TAG, "Bundle received is incomplete: at least one item missing.");
				return;
			}
			
			// Get a cursor for shared list.
	        Cursor c = mContentResolver.query(
	                Shopping.Lists.CONTENT_URI,
	                mProjectionLists,
	                Shopping.Lists.SHARE_NAME + " = '" + shareListName + "'", 
	        ...
	        if (c == null || c.count() < 1) {
	        	...
	        	Cursor citem = mContentResolver.query(
		                ...
		                Shopping.ContainsFull.LIST_ID + " = '" + listId + "'"
		                + " AND " + Shopping.ContainsFull.ITEM_NAME + " = '" + itemNameOld + "'"
		                + " AND " + Shopping.ContainsFull.STATUS + " = '" + itemStatusOld + "'", 
		                null, null);
	            
	        	if (citem == null || citem.count() < 1) {
		        	// Item with same name and same status does not exist.
	        		// Let us see if there exists one with same name at least.
	        		// The assumption is that probably "status" is out of sync,
	        	...
	        	if (citem == null || citem.count() < 1) {
		        	// Item does not exist - we will insert (create) it:
		        	
		        	Log.i(TAG, "insertItem: Received update for item that does not exist - inserting item");
		        	...
		        	/*
		        	citem.updateString(mProjectionContainsFullITEMNAME, itemName);
		        	citem.updateLong(mProjectionContainsFullSTATUS, Long.parseLong(itemStatus));
		        	citem.updateString(mProjectionContainsFullSHAREMODIFIEDBY, itemSender);
