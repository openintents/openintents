...
public class WifiConfigHelper {
	...
	public static String writeToString(WifiConfiguration wc,String authType) {
		StringBuffer sb= new StringBuffer();
		JSONObject jo= new JSONObject();
		try {
			jo.put("hiddenSSID", wc.hiddenSSID);
			jo.put("preSharedKey",wc.password);
			int wepkeyLen=wc.wepKeys.length;
			for(int i=0;i<wepkeyLen;i++)
			{
				final String key="WEPKey"+i;
				jo.put(key,wc.wepKeys[i]);
			}
			jo.put("SSID",wc.SSID);
			jo.put("BSSID",wc.BSSID);

			if(authType.equals("WEP"))
			{
				jo.put("KeyMgmt","NONE");
			}
