...
/**
 * 
 * Helper class used by {@link BridgeService} for handling the ACTION_QUICK_POST Intent.
 * 
 * @author berke.andras
 */
public class QuickPostHelper {
	...
	public void quickPost(Context context, Bundle parameterSet) {
		...
		// check if the QuickPostProvider stores the latest version of this
		// QuickPost source
		...
		if (c.moveToFirst()) {
			...
			// check if the version provided by the Intent is greater than the
			// stored value
			if (version > c.getLong(1)) {
				// newer version
				if (iconUri == null) {
					// caller didn't provided an icon. caller application's app
					// icon will be used
					parameterSet.putString(Actions.EXTRA_SOURCE_ICON_URI,
							UriUtils.appIconToUri(context, uid).toString());
				}
				sourceId = insertOrUpdateQuickPostSource(resolver,
						parameterSet, c.getLong(0));
			} else {
			...
			if (iconUri == null) {
				...
				// will be used
				parameterSet.putString(Actions.EXTRA_SOURCE_ICON_URI, UriUtils
						.appIconToUri(context, uid).toString());
			}
			sourceId = insertOrUpdateQuickPostSource(resolver, parameterSet,
					null);
		}
		c.close();
		...
		if (sourceId != null) {
			...
			// if the caller provided an EVENT_KEY, we have to check if the
			// associated
			// row is already stored.
			if (eventKey != null) {
				Uri rawEventUri = Uri.withAppendedPath(QuickPosts.SOURCE_URI,
						QuickPosts.QUICKPOST_RAW_EVENTS_PATH);

				where = QuickPostEventsTable.SOURCE_ID + " = " + sourceId
						+ " AND " + Events.EVENT_KEY + " = ?";
				String[] whereArgs = new String[] { eventKey };

				c = resolver.query(rawEventUri, new String[] { Events._ID },
	...
	private Long insertOrUpdateQuickPostSource(ContentResolver resolver,
		...
		values.put(QuickPostSourcesTable.INTERACT_ACTION_TITLE,
				interactActionTitle);
		...
		if (updateRow == null) {
			...
			int res = resolver.update(ContentUris.QuickPostSources, values,
					where, null);
	...
	private void insertQuickPostEvent(ContentResolver resolver, Long sourceId,
		...
		Originator originator = (Originator) parameterSet
				.get(Events.ORIGINATOR);
		...
		resolver.insert(Uri.withAppendedPath(QuickPosts.SOURCE_URI,
				Events.EVENTS_PATH), cv);
		markAsContacted(resolver, contactKey);
	}

	private void markAsContacted(ContentResolver resolver, String contactKey) {

		Uri contactUri = Contacts.lookupContact(resolver, Uri.withAppendedPath(
				Contacts.CONTENT_LOOKUP_URI, contactKey));
		Contacts.markAsContacted(resolver, android.content.ContentUris
				.parseId(contactUri));
	...
	public void removeQuickPosts(Context context, int uid) {
		...
		Cursor c = resolver.query(ContentUris.QuickPostSources,
				new String[] { QuickPostSourcesTable._ID }, where, null, null);
		if (c.moveToFirst()) {
		...
		if (sourceId != null) {
			// delete quickposts
			where = QuickPostEventsTable.SOURCE_ID + " = " + sourceId;
			resolver.delete(Uri.withAppendedPath(QuickPosts.SOURCE_URI,
					Events.EVENTS_PATH), where, null);
