...
public class FactoryTestProvider extends EventsProvider {
	...
	/**
	 * Class for storing the parameters of the test set 
	 * (size and event interval).
	 */
	public static class FactoryTestConfig {
		...
		public static FactoryTestConfig load(Context context) {
			...
			return new FactoryTestConfig(sp.getInt(PREF_TEST_SET_SIZE,
					DEFAULT_TEST_SET_SIZE), sp.getLong(PREF_EVENT_INTERVAL,
		...
		public void save(Context context) {
			...
			context.getSharedPreferences(PREF_FILE, Context.MODE_PRIVATE)
					.edit().putInt(PREF_TEST_SET_SIZE, testSetSize).putLong(
							PREF_EVENT_INTERVAL, eventInterval).commit();

			// parameters have been changed
			// notify ContentResolver to reload events
			context.getContentResolver().notifyChange(FactoryTest.EVENTS_URI,
					null);
		}
	}
	...
	protected Cursor queryEventsForContact(String lookupKey) {
		...
		FactoryTestConfig factoryTestConfig = FactoryTestConfig
				.load(getContext());

		MatrixCursor mc = new MatrixCursor(new String[] { Events._ID,
				Events.CONTACT_KEY, Events.EVENT_KEY, Events.MESSAGE,
				Events.ORIGINATOR, Events.PUBLISHED_TIME

		});

		String[] testMessages = getContext().getResources().getStringArray(
				R.array.factory_test);
		...
		for (int i = 0; i < factoryTestConfig.testSetSize; i++) {
			mc.addRow(new Object[] {
					...
					i % 2 == 0 ? Events.Originator.user
							: Events.Originator.contact,
					System.currentTimeMillis()
							- factoryTestConfig.eventInterval * (i + 1) });
		}
